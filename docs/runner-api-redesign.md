...
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/node/types.go` (→ `pkg/spi`: `AnyNode`, `AnyExecutionResult`, `BaseExecutionResult`, `ExecutionContext`, `As`/`MustAs`)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/node/unmarshal.go` (→ registry `DecodeNode`)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/extractors/factory.go` (→ finish registry, drop switch)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/node/assertion.go` (→ operators registry; `assertionWire` decoder moves to `pkg/kinds/request`)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/operators/types.go` (→ registered `Operator` values)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/engine/engine.go` + `execution.go` (→ retype to `spi`, add middleware; phase/skip logic unchanged)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/engine/skipped.go` (→ `NodeKind.NewSkipped`; reason codes frozen)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/engine/observer.go` (→ `Result` type rename, JSON unchanged)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/node/request_node.go` (→ `pkg/kinds/request`; `context.Context`-driven deadline)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/flow/flow.go` (→ `Parse(data, reg)` + `Builder`)
- `/Users/dev/nanostack-workspace/echopoint-runner/internal/ephemeral/runner.go` + `internal/runtime/runtime.go` (→ one `Runner`)
- `/Users/dev/nanostack-workspace/echopoint-runner/pkg/compatibility` + `pkg/assertions` (→ deleted)