// Package dynamicvars resolves {{$name:args}} template variables to generated
// fake data. The {{$...}} namespace is owned here; generators are backed by
// gofakeit for breadth. It satisfies the resolver the engine wires into
// node.Runtime.Vars (Resolve(name, args) (string, error)).
//
...
// mutex-guarded; under concurrent draws the order (and thus exact values) can
// vary, so values are unique-per-run, not yet identical on replay.
package dynamicvars
...
// Context holds the per-execution generator state and satisfies the dynamic
// resolver the engine expects.
type Context struct {
	mu      sync.Mutex
...
// New builds a Context seeded deterministically from the execution id, so the
// same execution replays the same stream of values.
...
// NewEphemeral builds a Context seeded from a fresh random source — unique per
// process, not reproducible. Use it for the default runtime, when no execution
// id is available; real executions should use New(executionID).
func NewEphemeral() *Context {
	var b [seedHashByteCount]byte
	_, _ = rand.Read(b[:])
	return New(hex.EncodeToString(b[:]))
}
...
// Entry documents one generator. Desc and Example feed discovery (docs, the CLI,
// and AI agents listing available variables).
type Entry struct {
	Name     string
	Category string
	Desc     string
